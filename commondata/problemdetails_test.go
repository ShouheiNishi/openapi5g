...
func TestProblemDetails(t *testing.T) {
	...
	regGoString := regexp.MustCompile("commondata\\.ProblemDetails\\{AccessTokenError:interface \\{\\}" + regNil + ", AccessTokenRequest:interface \\{\\}" + regNil + ", " +
		"Cause:" + regNonNilString + ", Detail:" + regNonNilString + ", Instance:" + regNilString + ", InvalidParams:\\[\\]commondata\\.InvalidParam" + regNil + ", " +
	...
	assert.Equal(t, "%!x(commondata.ProblemDetails="+goodString+")", fmt.Sprintf("%x", pd))

	pd = commondata.ProblemDetails{
		AccessTokenError: nrf_token.AccessTokenErr{
			Error: nrf_token.InvalidRequest,
		},
		AccessTokenRequest: nrf_token.AccessTokenReq{
	...
	type testType commondata.ProblemDetails
	goodGoString := "commondata.ProblemDetails" + strings.TrimPrefix(fmt.Sprintf("%#v", testType(pd)), "commondata_test.testType")
